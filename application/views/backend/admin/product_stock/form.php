        ...
        <div class="col-sm-6">
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    <?php if(isset($product_stock)): ?>
                        <form method="POST" action="<?php echo base_url('backend/admin/productstock/update/'. $product_stock->id) ?>" enctype="multipart/form-data">
                    <?php else: ?>
                        <form method="POST" action="<?php echo base_url('backend/admin/productstock/store') ?>" enctype="multipart/form-data">
                    <?php endif ?>
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control <?php echo form_error('product_id') ? 'is-invalid' : '' ?>" id="product_id" name="product_id" value="">
                                <?php 
                                $query = $this->db->query("SELECT products.*, stores.name as store_name FROM products LEFT JOIN stores ON products.store_id = stores.id");
                                $products = $query->result();
                                ?>

                                <option value=""></option>
                                <?php foreach($products as $product): ?>
                                    <option <?php echo set_value('product_id', isset($product_stock) ? $product_stock->product_id : null) == $product->id ? 'selected=""' : '' ?> value="<?php echo $product->id ?>"><?php echo $product->store_name .' - '. $product->name ?></option>
                                <?php endforeach ?>
                            </select>
                        ...
                        <div class="text-right">
                            <a href="<?php echo base_url('backend/admin/product') ?>" class="btn btn-secondary"><i class="fa fa-times"></i> Cancel</a>
